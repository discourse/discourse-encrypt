...
export default Ember.Controller.extend(ModalFunctionality, {
  onShow() {
    ...
    getIdentity()
      ...
      .then(() => {
        const privateKeys = this.model.user.encrypt_private
          ? JSON.parse(this.model.user.encrypt_private)
          : {};
        privateKeys[label] = true;
        this.set("model.user.encrypt_private", JSON.stringify(privateKeys));
      })
