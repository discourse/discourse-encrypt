...
describe DiscourseEncrypt::EncryptController do
  ...
  describe "#reset_user" do
    ...
    it "resets everything" do
      expect {
        post "/encrypt/reset.json", params: { user_id: user.id, everything: true }
      }.to change { TopicAllowedUser.count }.by(-1).and change { EncryptedTopicsUser.count }.by(
              -1,
    ...
    it "resets only keys" do
      expect { post "/encrypt/reset.json", params: { user_id: user.id } }.to not_change {
        TopicAllowedUser.count
      }.and not_change { EncryptedTopicsUser.count }.and change { UserEncryptionKey.count }.by(-1)
  ...
  describe "#posts" do
    ...
    it "fetches posts" do
      ...
      get "/encrypt/posts.json", params: { term: "after:#{filter_date}" }
      ...
      get "/encrypt/posts.json", params: { term: "before:#{filter_date}" }
  ...
  describe "#show_all_keys" do
    ...
    let!(:topic3) do
      Fabricate(
        :encrypt_topic,
        topic_allowed_users: [Fabricate.build(:topic_allowed_user, user: user2)],
      )
    end
  ...
  describe "#update_all_keys" do
    ...
    it "updates only if all topic keys are present" do
      ...
      put "/encrypt/rotate.json", params: { public: "public key" }
